...
(function () {
    ...
    // ─── Background Music ────────────────────────────
    const bgMusic = new Audio('audio/Ruins.mp3');
    bgMusic.loop = true;
    bgMusic.volume = 0.12; // moderate volume on home screen

    // Fade bgMusic volume smoothly
    function fadeBgMusic(targetVol, duration) {
    ...
    document.addEventListener('click', ensureBgMusic, { once: true });
    ...
    function onAudioEnded() {
        ...
        if (currentSection < SECTION_COUNT - 1) {
            ...
            // Finished all sections – scroll back to top
            isPlaying = false;
            started = false;
            updatePlayPauseUI();
            playbar.classList.remove('active');
            scrollToSection(-1); // scroll to home
            // Raise bg music volume back up
            fadeBgMusic(0.12, 1500);
        }
    }
    ...
    function startPresentation() {
        ...
        enterFullscreen();
        ensureBgMusic();
        // Lower bg music for section narration
        fadeBgMusic(0.01, 1000);
